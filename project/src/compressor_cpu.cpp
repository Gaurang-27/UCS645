...
void compress_image_rgb_cpu(const ImageRGB& in, ImageRGB& out, int quality,
	...
	auto compute_t0 = std::chrono::high_resolution_clock::now();
	...
	auto compute_t1 = std::chrono::high_resolution_clock::now();

	if (timings) {
		timings->saliency_ms = saliency_ms;
		timings->compute_ms = std::chrono::duration<double, std::milli>(compute_t1 - compute_t0).count();
		timings->total_ms = std::chrono::duration<double, std::milli>(compute_t1 - total_t0).count();
		timings->upload_ms = 0.0;
