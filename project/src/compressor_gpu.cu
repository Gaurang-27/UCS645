...
void compress_channel_gpu(const uint8_t* src, uint8_t* dst, int width, int height,
	const float* inv_block_scales, cudaStream_t stream)
{
	...
	k_dct8x8_quant_idct<<<grid, block, 0, stream>>>(src, dst, width, height, pitch, inv_block_scales);
...
void compress_image_rgb_gpu(const ImageRGB& in, ImageRGB& out,
	const QualityMapConfig& quality_map, CompressionStageTimings* timings)
{
	...
	auto compute_block_scales = [&](const ImageRGB& img) {
		...
		auto saliency_t0 = std::chrono::high_resolution_clock::now();

		const BlockImportanceMap importance = compute_block_importance(img, saliency_params, debug_ptr);
		auto saliency_t1 = std::chrono::high_resolution_clock::now();
		saliency_ms += std::chrono::duration<double, std::milli>(saliency_t1 - saliency_t0).count();
	...
	upload_ms += std::chrono::duration<double, std::milli>(upload_t1 - upload_t0).count();

	uint8_t* d_r_src = nullptr; uint8_t* d_g_src = nullptr; uint8_t* d_b_src = nullptr;
	uint8_t* d_r_dst = nullptr; uint8_t* d_g_dst = nullptr; uint8_t* d_b_dst = nullptr;
	...
	upload_ms += std::chrono::duration<double, std::milli>(alloc_t1 - alloc_t0).count();

	ScopedHostRegister host_in_r;
	ScopedHostRegister host_in_g;
	ScopedHostRegister host_in_b;
	ScopedHostRegister host_out_r;
	ScopedHostRegister host_out_g;
	ScopedHostRegister host_out_b;
	const bool use_async =
		host_in_r.try_register(const_cast<uint8_t*>(in.r.data()), N) &&
		host_in_g.try_register(const_cast<uint8_t*>(in.g.data()), N) &&
		host_in_b.try_register(const_cast<uint8_t*>(in.b.data()), N) &&
		host_out_r.try_register(out.r.data(), N) &&
		host_out_g.try_register(out.g.data(), N) &&
		host_out_b.try_register(out.b.data(), N);

	auto launch_channel = [&](cudaStream_t stream,
		const uint8_t* h_src, uint8_t* h_dst,
		uint8_t* d_src, uint8_t* d_dst,
		const float* d_scales) {
		if (use_async) {
			CUDA_CHECK(cudaMemcpyAsync(d_src, h_src, N, cudaMemcpyHostToDevice, stream));
			compress_channel_gpu(d_src, d_dst, in.width, in.height, d_scales, stream);
			CUDA_CHECK(cudaMemcpyAsync(h_dst, d_dst, N, cudaMemcpyDeviceToHost, stream));
		}
		else {
			CUDA_CHECK(cudaMemcpy(d_src, h_src, N, cudaMemcpyHostToDevice));
			compress_channel_gpu(d_src, d_dst, in.width, in.height, d_scales);
			CUDA_CHECK(cudaMemcpy(h_dst, d_dst, N, cudaMemcpyDeviceToHost));
		}
	};
	...
	auto init_events = []() {
		ChannelEvents e;
		CUDA_CHECK(cudaEventCreate(&e.h2d_start));
		CUDA_CHECK(cudaEventCreate(&e.h2d_end));
		CUDA_CHECK(cudaEventCreate(&e.kernel_start));
		CUDA_CHECK(cudaEventCreate(&e.kernel_end));
		CUDA_CHECK(cudaEventCreate(&e.d2h_start));
		CUDA_CHECK(cudaEventCreate(&e.d2h_end));
		return e;
	};

	auto destroy_events = [](ChannelEvents& e) {
		cudaEventDestroy(e.h2d_start);
		cudaEventDestroy(e.h2d_end);
		cudaEventDestroy(e.kernel_start);
		cudaEventDestroy(e.kernel_end);
		cudaEventDestroy(e.d2h_start);
		cudaEventDestroy(e.d2h_end);
	};

	if (use_async) {
		ScopedStream stream_r;
		ScopedStream stream_g;
		ScopedStream stream_b;
		ChannelEvents ev_r = init_events();
		ChannelEvents ev_g = init_events();
		ChannelEvents ev_b = init_events();

		auto launch_channel_timed = [&](cudaStream_t stream, ChannelEvents& ev,
			const uint8_t* h_src, uint8_t* h_dst,
			uint8_t* d_src, uint8_t* d_dst,
			const float* d_scales) {
			CUDA_CHECK(cudaEventRecord(ev.h2d_start, stream));
			CUDA_CHECK(cudaMemcpyAsync(d_src, h_src, N, cudaMemcpyHostToDevice, stream));
			CUDA_CHECK(cudaEventRecord(ev.h2d_end, stream));
			CUDA_CHECK(cudaEventRecord(ev.kernel_start, stream));
			compress_channel_gpu(d_src, d_dst, in.width, in.height, d_scales, stream);
			CUDA_CHECK(cudaEventRecord(ev.kernel_end, stream));
			CUDA_CHECK(cudaEventRecord(ev.d2h_start, stream));
			CUDA_CHECK(cudaMemcpyAsync(h_dst, d_dst, N, cudaMemcpyDeviceToHost, stream));
			CUDA_CHECK(cudaEventRecord(ev.d2h_end, stream));
		};

		launch_channel_timed(stream_r.stream, ev_r, in.r.data(), out.r.data(), d_r_src, d_r_dst, d_block_main);
		launch_channel_timed(stream_g.stream, ev_g, in.g.data(), out.g.data(), d_g_src, d_g_dst, d_block_chroma);
		launch_channel_timed(stream_b.stream, ev_b, in.b.data(), out.b.data(), d_b_src, d_b_dst, d_block_chroma);

		CUDA_CHECK(cudaStreamSynchronize(stream_r.stream));
		CUDA_CHECK(cudaStreamSynchronize(stream_g.stream));
		CUDA_CHECK(cudaStreamSynchronize(stream_b.stream));

		auto event_ms = [](cudaEvent_t start, cudaEvent_t end) {
			float ms = 0.0f;
			CUDA_CHECK(cudaEventElapsedTime(&ms, start, end));
			return static_cast<double>(ms);
		};
		upload_ms += std::max({
			event_ms(ev_r.h2d_start, ev_r.h2d_end),
			event_ms(ev_g.h2d_start, ev_g.h2d_end),
			event_ms(ev_b.h2d_start, ev_b.h2d_end)
		});
		compute_ms += std::max({
			event_ms(ev_r.kernel_start, ev_r.kernel_end),
			event_ms(ev_g.kernel_start, ev_g.kernel_end),
			event_ms(ev_b.kernel_start, ev_b.kernel_end)
		});
		download_ms += std::max({
			event_ms(ev_r.d2h_start, ev_r.d2h_end),
			event_ms(ev_g.d2h_start, ev_g.d2h_end),
			event_ms(ev_b.d2h_start, ev_b.d2h_end)
		});
		...
		launch_channel(nullptr, in.r.data(), out.r.data(), d_r_src, d_r_dst, d_block_main);
		launch_channel(nullptr, in.g.data(), out.g.data(), d_g_src, d_g_dst, d_block_chroma);
		launch_channel(nullptr, in.b.data(), out.b.data(), d_b_src, d_b_dst, d_block_chroma);
		compute_ms += 0.0;
		// In fallback mode, the blocking memcpy/launch path already contributes to total wall time.
	}
	auto done_t1 = std::chrono::high_resolution_clock::now();
	if (!use_async) {
		upload_ms += std::chrono::duration<double, std::milli>(done_t1 - upload_t0).count();
	}
	...
	if (timings) {
		timings->saliency_ms = saliency_ms;
		timings->upload_ms = upload_ms;
		timings->compute_ms = compute_ms;
		timings->download_ms = download_ms;
		timings->total_ms = std::chrono::duration<double, std::milli>(done_t1 - total_t0).count();
