...
struct CompressionStageTimings {
...
void compress_image_rgb_gpu(const ImageRGB& in, ImageRGB& out,
  const QualityMapConfig& quality_map = {}, CompressionStageTimings* timings = nullptr);
...
void compress_channel_gpu(const uint8_t* src, uint8_t* dst, int width, int height,
  const float* inv_block_scales = nullptr, cudaStream_t stream = nullptr);
