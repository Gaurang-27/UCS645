/*
 * ============================================================
 * Solution file for Assignment 8 — Problem 4
 * Implementations: Tiled GEMM, GEMM benchmark, MaxPool2x2,
 * BatchNorm (inference), Direct Conv2D.
 * Filename: 4.cu
 * ============================================================
 */
                            ...
                            int M, int N, int K)
{
    ...
    for (int t = 0; t < (K + TILE - 1) / TILE; t++) {
        ...
        tA[threadIdx.y][threadIdx.x] = (row < M && a_col < K)
            ? A[row * K + a_col] : 0.0f;

        /* Load tile of B */
        int b_row = t * TILE + threadIdx.y;
        tB[threadIdx.y][threadIdx.x] = (col < N && b_row < K)
            ? B[b_row * N + col] : 0.0f;
        ...
        for (int k2 = 0; k2 < TILE; k2++)
            sum += tA[threadIdx.y][k2] * tB[k2][threadIdx.x];
...
void diy_gemm_benchmark(cublasHandle_t handle)
{
    ...
    for (int s = 0; s < n_sizes; s++) {
        ...
        /* cuBLAS: note column-major ordering -> pass B,A and sizes accordingly */
        {
        ...
        double gflops = 2.0 * M * N * K / (cublas_ms / 1000.0 + 1e-12) / 1e9;
        ...
        for (int kh = 0; kh < kH; kh++) {
            for (int kw = 0; kw < kW; kw++) {
                ...
                if (ih >= 0 && ih < H && iw >= 0 && iw < W) {
                    int in_idx = ((n*C_in + ic)*H + ih)*W + iw;
                    int fil_idx = ((oc*C_in + ic)*kH + kh)*kW + kw;
                    sum += input[in_idx] * filter[fil_idx];
                }
            }
        }
    }
